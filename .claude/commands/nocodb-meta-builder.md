...
7. Use `--table` to export specific tables only.
   `node ./bin/noco-meta.js export --table "Posts,Authors"`
...
## v2 manifest conventions

- **SingleSelect / MultiSelect choices** must be objects: `[{"title": "A"}, {"title": "B"}]`. Plain strings `["A", "B"]` cause a 400 error on v2.
- **Link fields (`bt`)** can be declared on the child (many) side. Use `"relationType": "bt"` with `"relatedTable"` pointing to the parent (one) side.
- **All tables referenced by link fields** must appear in the manifest — even existing tables that need no new fields. Add them with `"create": false, "fields": []`.
- **Base context switching**: Use `node ./bin/noco-meta.js context` to interactively switch the active base, or `--base-id <id>` per command. No need to create separate profiles.
...
| `Duplicate table alias` on table create  | Orphan in NocoDB meta   | Check `SELECT * FROM nc_models_v2 WHERE title = '<name>'` and delete the orphaned row                |
| `Cannot read properties of undefined (reading 'dt')` on link create | Table lacks PK column | Ensure tables are created with an `Id` PK column (fixed in CLI — rebuild with `npm run build`) |
